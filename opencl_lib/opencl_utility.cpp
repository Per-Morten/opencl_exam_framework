...
namespace cp
{
    namespace opencl
    {
        ...
        char*
            read_program(const char* path)
        {
            FILE* file = fopen(path, "r");
            if (file == nullptr)
            {
                CP_ERROR("Unable to open file path")
            }
            ...
            char* source = static_cast<char*>(calloc(size, sizeof(char)));
            ...
            bool invalid = (char_read < static_cast<size_t>(size) &&
                            feof(file) == 0 &&
                            ferror(file) != 0);
